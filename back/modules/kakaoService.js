    ...
    app.post('/admin/allUser', async(req,res) => {
        ...
        if(refresh){
            ...
            try {
                ...
                console.log('=========start=========');
                users.forEach(el => {
                    User.findOne({id: el}).exec(async function(err,user){
                        if(!user){
                            ...
                            userSave.id = el;
                            userSave.connected_at = result.data.connected_at
                            userSave.properties = result.data.properties
                            userSave.kakao_account =result.data.kakao_account
                            userSave.auth = 'user';
